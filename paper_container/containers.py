...
class ContainersReady:
    ...
    @property
    def new_experiment_base_image(self) -> Dict[str, Union[Optional[Image], BuildConfig, str]]:
        image_name = "paper-experiments-llm"
        tag = "latest"
        full_name = f"{image_name}:{tag}"
        if full_name not in self._image_manager.images.keys():
            self._add_image(
                image_name=image_name,
                tag=tag,
                config=self._configs.exp_base_config(),
            )
        return self._image_manager.images[full_name]
