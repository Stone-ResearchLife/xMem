...
class CommProfiler(BaseProfiler):
    ...
    def result_str(self, sep: str = "\n"):
        ...
        append(
            "average bandwidth: {}".format(
                _format_bandwidth(self.total_comm_vol, self.total_cuda_time)
            )
        )
        ...
        for location, event in show_list:
            ...
            append(
                row_format.format(
                    ...
                    "{:.1f}%".format(
                        event.self_cuda_time / self.total_cuda_time * 100.0
                    ),
    ...
    def close_profiler(self, group=None):
        ...
        if self.profiler.enabled and dist.get_world_size(group) > 1:
            ...
            buffer = torch.tensor(
                [current_comm_event.self_cuda_time], device=get_current_device()
            )
