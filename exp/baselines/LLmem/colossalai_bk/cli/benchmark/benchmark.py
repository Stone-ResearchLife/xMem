...
import colossalai
from colossalai.cli.benchmark.utils import (
    find_all_configs,
    get_batch_data,
    profile_model,
)
...
def run_benchmark(args: Config) -> None:
    ...
    if args.gpus <= 1:
        click.echo(
            "Warning: tensor parallel will be activated with at least 2 devices."
        )
...
def run_dist_profiling(
    ...
    for config, port in zip(config_list, port_list):
        ...
        if (
            config.parallel.tensor.mode == "1d"
            and hyperparams.dimension % config.parallel.tensor.size != 0
        ):
            click.echo(
        ...
        fwd_time, bwd_time, max_allocated, max_cached = profile_model(
            model=model,
        ...
        if gpc.get_global_rank() == 0:
            config_str = ", ".join(
                [f"{k}: {v}" for k, v in config.parallel.tensor.items()]
            )
