...
def colo_cross_entropy(
    ...
    assert (
        isinstance(weight, ColoTensor)
        or isinstance(target, ColoTensor)
        or isinstance(input_tensor, ColoTensor)
    )
    pg = (
        input_tensor.get_process_group()
        if isinstance(input_tensor, ColoTensor)
        else isinstance(target, ColoTensor)
    )
    ...
    if input_tensor.is_replicate():  # Input is gathered
        assert target.is_replicate() and (
            weight is None or weight.is_replicate()
        ...
        if input_tensor.is_shard_1dcol():
            ...
            output = VocabParallelCrossEntropyLoss1D()(
                input_tensor,
                target,
