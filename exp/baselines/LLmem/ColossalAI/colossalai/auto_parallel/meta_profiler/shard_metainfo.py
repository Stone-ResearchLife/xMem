...
class ShardMetaInfo:
    ...
    def __init__(
        self, strategy: ShardingStrategy = None, target: Callable = None
    ) -> None:
    ...
    def compute_sharded_opdata(
        ...
        if isinstance(sharding_spec, ShardingSpec):
            ...
            assert len(data) == len(
                sharding_spec
            ), f"Length of data and sharding spec should be the same."
            sharded_data = []
    ...
    def compute_shard_metainfo(self):
        ...
        args = [
            self.compute_sharded_opdata(k, v)
            for k, v in self._strategy.sharding_specs.items()
        ]
