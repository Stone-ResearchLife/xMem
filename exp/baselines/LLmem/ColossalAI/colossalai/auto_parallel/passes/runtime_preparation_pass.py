...
def solution_annotation_pass(
    ...
    for node_index, (node, strategy_index) in enumerate(zip(nodes, solution)):
        ...
        setattr(
            node,
            "sharding_spec",
            strategies_vector[strategy_index].get_sharding_spec_by_name(str(node)),
        )
    ...
    for index, node in enumerate(nodes):
        ...
        if node.op == "get_attr":
            ...
            ), f"sharing weight is not supported in current version."
    ...
    for node in nodes:
        if node.op != "placeholder":
            with mod_graph.inserting_before(node):
                ...
                origin_specs_node = mod_graph.create_node(
                    "placeholder", target="origin_node_sharding_spec_dict"
                )
                comm_actions_dict_node = mod_graph.create_node(
                    "placeholder", target="comm_actions_dict"
                )
            break
    return (
        gm,
