...
def train_epoch(
    ...
    with tqdm(
        dataloader, desc=f"Epoch [{epoch + 1}]", disable=not coordinator.is_master()
    ) as pbar:
        for batch in pbar:
            # Forward
...
def train():
    ...
    compute_spec = [
        ...
        ComputeSpec(ComputePattern.TP2D),
        ComputeSpec(ComputePattern.TP2P5D),
        ComputeSpec(ComputePattern.TP3D),
    ]
    ...
    if mode == "2.5d":
        ...
        colossalai.launch_from_torch(
            config=dict(
                parallel=dict(
                    data=1, pipeline=1, tensor=dict(size=tp_size, mode=mode[tp_dim])
                )
            )
        )
    ...
    for epoch in range(config["epochs"]):
        train_epoch(
            ...
            tp_dim=tp_dim,
            batch_size=config["batch_size"],
        )  # tp_degree=tp_degree, dims=dims_l)
