...
from colossalai.zero.gemini.memory_tracer import (
    MemStatsCollector,
...
from colossalai.zero.legacy.gemini.tensor_placement_policy import (
        ...
        if self._use_memory_tracer:
            ...
            if gpc.get_global_rank() == 0:
                with open(filename, "w+") as f:
                    f.write(
                        f"cuda reserved {torch.cuda.memory_reserved(get_current_device()) / 1e9} GB\n"
                    )
                    f.write(
                        f"cuda max allocated {torch.cuda.max_memory_allocated(get_current_device()) / 1e9} GB\n"
                    )
    ...
    def _update_memstats(self):
        if self._memstats_collector:
            ...
            self._cuda_margin_space = (
                colo_device_memory_capacity(get_current_device())
