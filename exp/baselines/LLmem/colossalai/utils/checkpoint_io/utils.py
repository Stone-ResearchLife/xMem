...
def shard_checkpoint(
    ...
    param_to_os: Optional[dict] = None,
    ...
    if not has_optimizer:
        return model_shards, []
    optimizer_sharder = OptimizerCheckpointSharder(
        max_shard_size, optimizer_state_dict["param_groups"]
    )
...
def build_checkpoints(
    ...
    if len(model_state_dict) == 0:
        ...
        return [], [], meta
    model_checkpoints, optimizer_checkpoints = shard_checkpoint(
        max_size, model_state_dict, optimizer_state_dict, param_to_os
    )
...
def optimizer_load_state_dict(
    optimizer: Optimizer, state_dict: dict, strict: bool = False
) -> None:
