...
def unflatten_zero_param(
    tensors: List[Tensor], dist_metas: List[ParamDistMeta]
) -> Tensor:
    assert len(tensors) > 0 and len(dist_metas) > 0 and len(tensors) == len(dist_metas)
    for dist_meta in dist_metas[1:]:
        assert (
            dist_meta.zero_meta == dist_metas[0].zero_meta
    ...
    tensors = [
        t[1] for t in sorted(zip(dist_metas, tensors), key=lambda tp: tp[0].dp_rank)
    ]
    assert numel == sum(
        t.numel() for t in tensors
...
def gather_tp_param(tensors: List[Tensor], dist_metas: List[ParamDistMeta]) -> Tensor:
    ...
    for dist_meta in dist_metas[1:]:
        assert (
            dist_meta.tp_meta == dist_metas[0].tp_meta
    ...
    assert (
        dist_meta.tp_world_size == total_parts
    ), f"Expect prod(tp_num_parts) == tp_world_size, got {total_parts} and {dist_meta.tp_world_size}."
    shard_info = sorted(
        zip(dist_meta.tp_shard_dims, dist_meta.tp_num_parts),
        key=lambda t: t[0],
        reverse=True,
    )
...
def validate_parallel_info(dist_metas: List[ParamDistMeta]) -> None:
    ...
    for dist_meta in dist_metas[1:]:
        assert (
            dist_meta.dp_world_size == dist_metas[0].dp_world_size
        ), "Expect all dist meta have the same dp_world_size"
        assert (
            dist_meta.tp_world_size == dist_metas[0].tp_world_size
        ), "Expect all dist meta have the same tp_world_size"


def deduplicate_params(
    tensors: List[Tensor], dist_metas: List[ParamDistMeta]
) -> Tuple[List[Tensor], List[ParamDistMeta]]:
    unique_dist_meta = []
    unique_idx = []
...
def merge_param(tensors: List[Tensor], dist_metas: List[ParamDistMeta]) -> Tensor:
    ...
    assert (
        len(tensor_dict) == dist_metas[0].tp_world_size
    ), f"Expect {dist_metas[0].tp_world_size} ranks, got {len(tensor_dict)}"
    for tp_rank in tensor_dict.keys():
        unflattened_tensors.append(
            unflatten_zero_param(tensor_dict[tp_rank], dist_meta_dict[tp_rank])
        )
    return gather_tp_param(
        unflattened_tensors,
        [dist_meta_list[0] for dist_meta_list in dist_meta_dict.values()],
    )
...
def split_tp_param(tensor: Tensor, redist_meta: ParamRedistMeta) -> List[Tensor]:
    if not redist_meta.used_tp:
        assert (
            redist_meta.tp_world_size == 1
        ), "Expect tp_world_size == 1, when no tp meta provided."
        return [tensor]
    total_parts = prod(redist_meta.tp_num_parts)
    assert (
        redist_meta.tp_world_size == total_parts
    ), f"Expect prod(tp_num_parts) == tp_world_size, got {total_parts} and {redist_meta.tp_world_size}."
    shard_info = sorted(
        zip(redist_meta.tp_shard_dims, redist_meta.tp_num_parts), key=lambda t: t[0]
    )
    tensors = [tensor]
    for dim, num_parts in shard_info:
        buffer = []
...
def flatten_zero_param(tensor: Tensor, redist_meta: ParamRedistMeta) -> List[Tensor]:
    ...
    tensors: List[Optional[Tensor]] = [
        ...
        for _ in range(redist_meta.zero_start_dp_rank)
    ]
