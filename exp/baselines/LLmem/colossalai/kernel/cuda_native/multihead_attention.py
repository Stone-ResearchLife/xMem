...
class MultiHeadAttention1DFunc(Function):
    ...
    def forward(
        ...
        forward_func = (
            cuda_module.multihead_attention_fw_fp16
            if config.fp16
    ...
    def backward(ctx, grad_output):
        ...
        cuda_module = colossal_multihead_attention
        backward_func = (
            cuda_module.multihead_attention_bw_fp16
            if ctx.config.fp16
            else cuda_module.multihead_attention_bw_fp32
        )
            ...
            with torch.no_grad():
                self.in_proj_weight.copy_(
                    attn_qkvw_global.view(3, hs, hs)[
                        :,
                        int(hs * rank_in_pg / self.pg_size) : int(
                            hs * (rank_in_pg + 1) / self.pg_size
                        ),
                        :,
                    ]
                )
