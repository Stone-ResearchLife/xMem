...
def colo_addmm_1Drow(
    ...
    mat1 = mat1.redistribute(
        ShardSpec([-1], [mat2.get_tp_world_size()]), mat2.get_process_group()
    )
    ...
    output = ColoTensor.from_torch_tensor(
        output, spec=ColoTensorSpec(input_tensor.get_process_group())
    )
    return output


def colo_addmm_1Dcol(
    input_tensor: ColoTensor,
    mat1: ColoTensor,
    mat2: ColoTensor,
    beta: Number,
    ...
    output_spec = ColoTensorSpec(
        ...
        ShardSpec([-1], [mat2.get_tp_world_size()]),
    ...
    **kargs
    ...
    if not mat2.has_compute_spec():  # No Model Parallel Applied
        ...
        ret_tensor = ColoTensor.from_torch_tensor(
            tensor=torch.addmm(
                input_tensor, mat1, mat2, beta=beta, alpha=alpha, **kargs
            ),
        ...
        if mat2.is_shard_1drow() and input_tensor.is_replicate():
            mode = "row"
        elif mat2.is_shard_1dcol() and (
            input_tensor.is_shard_1dcol() or input_tensor.is_shard_1drow()
        ):
            mode = "col"
        else:
