...
def colo_embedding_1Dcol(
    ...
    output_parallel = F.embedding(
        input_tensor,  # 1st input_tensor size: [8, 169]
...
def colo_embedding_1Drow(
    ...
    partial_output[input_mask, :] = 0.0
    # Reduce across all the model parallel GPUs.
    output = reduce_input(partial_output, weight.get_process_group())
    output = ColoTensor.from_torch_tensor(
        output, spec=ColoTensorSpec(weight.get_process_group(), ReplicaSpec())
    )

    return output


def colo_embedding_1d(
    mode: str,
    input_tensor: ColoTensor,
    weight: ColoTensor,
    padding_idx: Optional[int] = None,
