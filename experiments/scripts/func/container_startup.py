    ...
    def runtime_config(self, args: argparse.Namespace) -> RuntimeConfig:
        ...
        if args.volumes is not None:
            ...
            for volume in args.volumes:
                ...
                _volumes[src] = {"bind": dest, "mode": "rw"}
...
class PytorchModelTemplate(AbcTemplate):
    ...
    def runtime_config(self, args: argparse.Namespace) -> RuntimeConfig:
        ...
        if args.volumes is not None:
            for volume in args.volumes:
                src, dest = volume.split(":")
                _params["volumes"][src] = {"bind": dest, "mode": "rw"}
        if args.subnet is not None and args.ipv4 is not None:
            _params["subnet"] = args.subnet
            _params["ipv4"] = args.ipv4
        return RuntimeConfig(**_params)
...
def get_arg_parser():
    parser = argparse.ArgumentParser(
        description="Runtime configuration for container execution"
    )
    ...
    parser.add_argument(
        "--name", type=str, default=None, help="Name of the container (default: None)"
    )
    parser.add_argument(
        "--force-rebuild",
        action="store_true",
    ...
    parser.add_argument(
        "--build-only", action="store_true", default=False, help="Build the image only"
    )
