...
class RuntimeConfig(BaseModel):
    image_name: str = Field(
        default="model-runner", title="Image Name", description="Name of the image"
    )
    name: Optional[str] = Field(
        default=None, title="Name", description="Name of the container"
    )
    force_rebuild: bool = Field(
        default=False, title="Force Rebuild", description="Force rebuild the image"
    )
    ...
    ipv4: Optional[str] = Field(
        default=None, title="IPv4", description="IPv4 address for the container"
    )
    temp_dir: Path = Field(
        default=Path(fetch_temp_folder()),
    ...
    build_config: Optional[BuildConfig] = Field(
        default=None,
        title="Build Config",
        description="Build configuration for the image",
    )
    ...
    def set_subnet(self, subnet: str, ipv4: str):
        ...
        try:
            ...
            if not verify_ip_in_subnet(
                ipv4, docker_network.attrs["IPAM"]["Config"][0]["Subnet"]
            ):
    ...
    def to_json(self):
        ...
        if self.gpus is not None:
            _params["device_requests"] = [
                docker.types.DeviceRequest(
                    driver="nvidia", device_ids=self.gpus, capabilities=[["gpu"]]
                )
            ]
        ...
        if self.entrypoint is not None:
            _params["entrypoint"] = [
                str(_entrypoint) for _entrypoint in self.entrypoint
            ]
...
class ContainerRunner:
    ...
    def create(self):
        ...
        if _image.name != self._config.image_name:
            logger.info(
                f"Image name is different from the config: {_image.name} != {self._config.image_name}"
            )
