...
def save_checkpoint(
    ...
    **kwargs
    ...
    if rank == 0:
        ...
        torch.save(
            save_state, path + "/epoch_{}_model.pth".format(epoch), *args, **kwargs
        )
    ...
    if optimizer is not None:
        ...
        if rank == 0:
            ...
            torch.save(
                save_state, path + "/epoch_{}_optim.pth".format(epoch), *args, **kwargs
            )
...
def load_checkpoint(
    ...
    if rank == 0:
        load_state = torch.load(
            path + "/epoch_{}_model.pth".format(epoch), **torch_load_kwargs
        )
    ...
    if optimizer is not None:
        ...
        if rank == 0:
            colo_checkpoint = torch.load(
                path + "/epoch_{}_optim.pth".format(epoch), **torch_load_kwargs
            )
