...
class DiskCheckpointWriter(CheckpointWriter):
    ...
    def write(self, name: str, state_dict: dict) -> None:
        ...
        if os.path.exists(path) and not self.overwrite:
            raise RuntimeError(
                f'Save error: Checkpoint "{path}" exists. (overwrite = False)'
            )
            ...
            if self.is_distributed:
                for i in range(self.world_size):
                    global_meta["meta"].append(
                        META_CKPT_FILE_NAME.replace(".bin", f"-rank{i}.bin")
                    )
            else:
    ...
    def _get_checkpoint_name(
        self, base_name: str, shard_idx: Optional[int] = None
    ) -> str:
    ...
    def save_model(self, model_checkpoint: dict) -> None:
        ...
        name = self._get_checkpoint_name(
            MODEL_CKPT_FILE_NAME, len(self.model_checkpoint_names)
        )
    ...
    def save_optimizer(self, optimizer_checkpoint: dict) -> None:
        ...
        name = self._get_checkpoint_name(
            OPTIM_CKPT_FILE_NAME, len(self.optimizer_checkpoint_names)
        )
