...
def _all_gather(tensor, comm_spec):
    ...
    """
    process_groups_list = comm_spec.device_mesh.process_groups_dict[
    ...
    for rank_list, process_group in process_groups_list:
        if dist.get_rank() in rank_list:
            tensor_list = [
                ...
                for _ in range(
                    comm_spec.device_mesh.mesh_shape[comm_spec.logical_process_axis]
                )
            ]
...
def _all_to_all(tensor, comm_spec):
    ...
    for rank_list, process_group in process_groups_list:
        if dist.get_rank() in rank_list:
            ...
            new_shape[comm_spec.shard_dim] = new_shape[comm_spec.shard_dim] // len(
                rank_list
            )
            ...
            output = torch.cat(
                tuple(output_tensor_list), comm_spec.gather_dim
            ).contiguous()
            return output
    ...
    for rank_list, process_group in process_groups_list:
        if dist.get_rank() in rank_list:
            if not tensor.is_contiguous():
                tensor = tensor.contiguous()
            dist.all_reduce(
                tensor, op=ReduceOp.SUM, group=process_group, async_op=async_op
            )
            return tensor
