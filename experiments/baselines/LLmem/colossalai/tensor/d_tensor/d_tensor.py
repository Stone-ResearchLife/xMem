...
class DTensor(torch.Tensor):
    ...
    def __new__(cls, local_tensor, layout):
        return torch.Tensor._make_subclass(
            cls, local_tensor, local_tensor.requires_grad
        )
    ...
    def layout_convert(self, target_layout):
        ...
        """
        self.local_tensor = layout_converter.apply(
            self.local_tensor, self.dist_layout, target_layout
        )
    ...
    def _apply_layout(self):
        ...
        source_layout = Layout(
            device_mesh=self.dist_layout.device_mesh,
            device_type=self.dist_layout.device_type,
            sharding_spec=source_spec,
            entire_shape=self.entire_shape,
        )
        self.local_tensor = layout_converter.apply(
            self.local_tensor, source_layout, self.dist_layout
        )
...
def distribute_module(
    module: torch.nn.Module, partition_fn: Optional[callable] = None
) -> torch.nn.Module:
    """
