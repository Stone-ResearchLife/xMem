    ...
    def __init__(self, expert_cls: Type[nn.Module], num_experts: int, **expert_args):
        ...
        with seed(ParallelMode.TENSOR):
            self.experts = nn.ModuleList(
                [expert_cls(**expert_args) for _ in range(self.num_local_experts)]
            )
        ...
        if dp_rank == 0:
            ...
            for i in range(self.num_total_experts):
                ...
                for name, param in comm_module.named_parameters():
                    dist.broadcast(
                        param.data, src=source_rank, group=self.dist_info.ep_group
                    )
                    if ep_rank == 0:
...
class FFNExperts(MoeExperts):
    ...
    def __init__(
        ...
        self.w1 = nn.Parameter(
            torch.empty(
                self.num_local_experts, d_model, d_ff, device=get_current_device()
            )
        )
        self.b1 = nn.Parameter(
            torch.empty(self.num_local_experts, 1, d_ff, device=get_current_device())
        )

        self.w2 = nn.Parameter(
            torch.empty(
                self.num_local_experts, d_ff, d_model, device=get_current_device()
            )
        )
        self.b2 = nn.Parameter(
            torch.empty(self.num_local_experts, 1, d_model, device=get_current_device())
        )
...
class TPExperts(MoeExperts):
    ...
    def __init__(
        ...
        assert (
            d_ff % MOE_CONTEXT.max_ep_size == 0
        ), "d_ff should be divide by maximum expert parallel size"

        p_ff = d_ff // MOE_CONTEXT.max_ep_size

        self.w1 = nn.Parameter(
            torch.empty(num_experts, d_model, p_ff, device=get_current_device())
        )
        self.b1 = nn.Parameter(
            torch.empty(num_experts, 1, p_ff, device=get_current_device())
        )

        self.w2 = nn.Parameter(
            torch.empty(num_experts, p_ff, d_model, device=get_current_device())
        )
        self.b2 = nn.Parameter(
            torch.empty(num_experts, 1, d_model, device=get_current_device())
        )
