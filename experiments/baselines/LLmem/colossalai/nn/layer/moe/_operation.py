...
class AllGather(torch.autograd.Function):
    ...
    @staticmethod
    def forward(
        ctx: Any, inputs: Tensor, group: Optional[ProcessGroup] = None
    ) -> Tensor:
...
class MoeDispatch(torch.autograd.Function):
    ...
    def backward(ctx, output_grad):
        ...
        d_tokens = moe.dispatch_backward(
...
class MoeCombine(torch.autograd.Function):
    ...
    def backward(ctx, tokens_grad):
        ...
        cb_grad = (
            ...
            if tokens_grad.dtype is torch.float16
            else tokens_grad
        )
        cb_input = expert_tokens.to(torch.float32) if ctx.fp16_flag else expert_tokens
        d_expert, d_logits = moe.combine_backward(
            ctx.s, ctx.e, ctx.c, ctx.h, cb_grad, cb_input, logits, mask, dest_idx
        )
