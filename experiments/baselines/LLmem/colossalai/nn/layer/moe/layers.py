...
from colossalai.zero.legacy.init_ctx import (
...
class MoeLayer(nn.Module):
    ...
    def __init__(
        self, dim_model: int, num_experts: int, router: MoeRouter, experts: MoeExperts
    ):
        super().__init__()
        ...
        self.use_kernel = (
            True if COL_MOE_KERNEL_FLAG and MOE_CONTEXT.use_kernel_optim else False
        )
    ...
    def a2a_process(self, dispatch_data: torch.Tensor):
        ...
        expert_input = expert_input.reshape(
            self.ep_size, self.num_local_experts, -1, self.d_model
        )
    ...
    def forward(self, inputs: torch.Tensor) -> Tuple:
        ...
        route_result_list = self.router(
            inputs=gate_output, use_kernel=self.use_kernel, ep_group=self.ep_group
        )
...
class MoeModule(nn.Module):
        ...
        **expert_args
        ...
        if use_residual:
            ...
            with no_shard_zero_context():
                self.residual_combine = nn.Linear(
                    dim_model, 2, device=get_current_device()
                )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_residual:
            ...
            output = (
                moe_output * combine_coef[..., 0:1]
                + residual_output * combine_coef[..., 1:]
            )
        else:
            output = moe_output
