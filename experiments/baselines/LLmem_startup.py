...
class SizeEstimator(object):
    ...
    def get_output_sizes(self):
        ...
        for i in range(1, len(mods)):
            if "DecoderLayer" in mods[i]._get_name():  # or 'BertLayer'
                ...
                repeat_layer += 1
                block_first = 1
            if not mods[i]._get_name() in [
                "Embedding",
                "Linear",
            ]:  # , 'ReLU', 'LayerNorm', 'GELUActivation']:
                continue
            if i == pr_out:
                continue
            ##################### For BERT #####################
            ...
            if mods[i]._get_name() in ["Linear"]:
                if block_first == 1:
                    ...
                    # ============ new code =================
                    # to avoid the shape mismatch error in `out = m(input_)`
                    # the original code does not handle this error,
                    # and tensor.view is a common function used to reshape the tensor before linear layer
                    continue
            #####################################################
            ...
            if i == len(param_sizes) - 1:
                ...
                if not self.tp:
                    if self.gpu_n > 1:  # ZeRO-3 Optimizer
                        ...
                        if bytes % self.base_size != 0:
                            bytes = (
                                int(bytes / self.base_size) * self.base_size
                                + self.base_size
                            )
                        total_bytes += bytes
                elif (
                    self.tp and self.gpu_n > 1 and self.tp != self.gpu_n
                ):  # DP + TP (Hybrid parallelism)
                    bytes = bytes * (
                        (self.gpu_n - self.tp) / self.gpu_n - 1 / self.gpu_n
                    )
                    if bytes % self.base_size != 0:
                        bytes = (
                            int(bytes / self.base_size) * self.base_size
                ...
                if bytes % self.base_size != 0:
                    bytes = (
                        int(bytes / self.base_size) * self.base_size + self.base_size
                    )
    ...
    def calc_input_bytes(self):
        ...
        if self.input_bytes % self.base_size != 0:
            self.input_bytes = (
                int(self.input_bytes / self.base_size) * self.base_size + self.base_size
            )
    ...
    def bs_search(self):
        ...
        while True:
            ...
            total_mem += (
                self.param_bytes + self.input_bytes + total_bytes + last_part
            ) / (1024**2)
...
def main(
    ...
    se = SizeEstimator(
        model, batch, real_bs, bytes, bytes_input, gpu_n, tp, lm_fp32, m_total
    )
    se.get_output_sizes()
    real_mem, real_bs = se.estimate_size(init_size)
    if output is not None:
        _json_data = {"real_mem": real_mem, "real_bs": real_bs}
        output = Path(output)
        if output.is_dir():
            output = output.joinpath("output.json")
