...
class Schedtune:
    def __init__(
        ...
        self.conf_dir = conf_dir or Path(
            os.path.join(os.path.dirname(os.path.realpath(__file__)), "schedtune")
        )
    ...
    def estimate(self):
        ...
        if self.option == "1":
                ...
                tm = train_mem_pred.predict(
                    [
                        [
                            ...
                            float(self.parameters),
                            float(self.inputsize),
                            gpu[1],
                            gpu[0],
                            gpu[4],
                        ]
            ...
            if "inference" in self.jobname:
                infer_mem_pred = joblib.load(
                    conf_dir.joinpath("infer_mem_RFR_5params.joblib")
                )
            ...
            if "batchsize" in self.jobname:
                train_mem_pred = joblib.load(
                    conf_dir.joinpath("train_mem_RFR_5params.joblib")
                )
                ##CUDA_cores,MemoryBW_GBps,Memory_clock_speed_MHz,Tensor_cores,SM_count
            ...
            if "inference" in self.jobname:
                infer_mem_pred = joblib.load(
                    conf_dir.joinpath("infer_mem_RFR_3params.joblib")
                )
