...
class XMemEvaluation(InterfaceApp):
    ...
    def _execute(
        ...
        container = estimator.run_image(
            image_name=self._image_name, name=name, command=command
        )
        return container
        ...
        for opt in tqdm.tqdm(optimiser):
            for batch_size in tqdm.tqdm(
                ...
                for model in tqdm.tqdm(models):
                    self._temp_dir = original_tmp.joinpath(
                        f"anova-{model}-{opt}-{batch_size}-{device}"
                    )
                    self._temp_dir.mkdir(parents=True, exist_ok=True)
    ...
    def monte_carlo(self, times: int = 10):
        ...
        for i in tqdm.tqdm(range(times)):
            ...
            self._temp_dir = original_tmp.joinpath(
                f"MonteCarlo-{model}-{opt}-{batch_size}-{device}"
            )
            self._temp_dir.mkdir(parents=True, exist_ok=True)
            _contain = self._execute(
                model=model,
