...
class ExperimentPlot:
        ...
        if str(data_dir) not in self._cache.keys():
            ...
            for eva_file in evaluates_files:
                ...
                for key, value in eva_data.items():
                    ...
                    copy_basic_info["2nd_real_oom"] = value.get("2nd verification", {}).get("oom")
                    copy_basic_info["2nd_error"] = value.get("2nd verification", {}).get("error", value["error"])
                    if value["real_oom"] == False and value["correct_estimation"] == True and copy_basic_info["2nd_real_oom"] == False:
                    ...
                    elif value["real_oom"] == True and value["correct_estimation"] == True:
    ...
    def _add_four_quadrant(self, fig, quadrant_thresholds: tuple, max_y: int, font_size: int = 45):
        # four-quadrant
    ...
    def plot_assessment_result_between_snapshot_and_simulation(
        ...
        if str(data_dir) not in self._cache.keys():
            ...
            for index, p in enumerate(pickles):
                ...
                memory_data = self._compare_snapshot_and_allocator_simulator(snapshot_analyser, _result_allocate)
                # Convert to GB
    ...
    def plot_relative_error_in_box_diagram_with_verification_data(
        ...
        fig.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor='LightGray', zeroline=False, titlefont=self.font, range=yrange, tickfont=dict(size=tickfont_size)),
            xaxis=dict(showgrid=False, titlefont=self.font, tickfont=dict(size=tickfont_size)),
            title_x=0.5,
            plot_bgcolor='white',
    ...
    def plot_probability_estimation_vs_error_scatter_diagram_model_base(
        ...
        fig = px.scatter(
            ...
            hover_data=['Correct_Estimation_False_Count', 'Correct_Estimation_True_Count', 'Mean_Error'],  # Additional info on hover
            color_discrete_map=self._color_scheme,
        )
        ...
        fig = self._add_four_quadrant(fig, (20, 20), correctness_counts['Mean_Error'].max(), font_size=quadrant_font_size)
    ...
    def plot_cdf_random_test_estimator_performance_score(
        ...
        for tool_name, group in grouped.groupby('tool'):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    line=dict(color=self._color_scheme.get(tool_name), dash="dashdot", width=line_width),
                )
            )
        ...
        for tool_name, group in grouped.groupby('tool'):
            ...
            fig.add_trace(go.Scatter(
                ...
                line=dict(color=self._color_scheme.get(tool_name), width=line_width),
            ))
        ...
        fig.update_layout(
            ...
            yaxis_title="CDF",
            yaxis=dict(showgrid=True, gridcolor='LightGray', zeroline=False, titlefont=self.font, range=[0, 1], tickfont=dict(size=tickfont_size)),
            xaxis=dict(showgrid=False, titlefont=self.font, range=[0, 1], tickfont=dict(size=tickfont_size)),
    ...
    def plot_random_test_memory_saved_bar_chart(
        ...
        fig.update_layout(
            ...
            yaxis=dict(showgrid=True, gridcolor='LightGray', zeroline=False, titlefont=self.font, tickfont=dict(size=tickfont_size)),
            xaxis=dict(showgrid=False, titlefont=self.font, tickfont=dict(size=tickfont_size)),
            height=default_height,  # default height
            width=default_width,  # default width
            font=self.font,
    ...
    def summarize_data(
        ...
        correctness_counts.columns = groupby_list + ['Correct_Estimation_False_Count', 'Correct_Estimation_True_Count']
        correctness_counts['probability'] = (correctness_counts['Correct_Estimation_False_Count'] / (
        ...
        second_perf_score = df.groupby(['tool', 'model', "optimiser"]).agg(
        ...
        for tool_name, group in second_perf_score.groupby('tool'):
            group['combined_score'] = 0.7 * (1 - group['second_oom_success_rate']) + 0.3 * group[
                'average_second_error']
        ...
        for field in field_list:
            ...
            xmem_value = merged_df[merged_df['tool'] == 'xMem (this paper)'][field].values[0]
