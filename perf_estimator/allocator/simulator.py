...
class AllocatorSim:
    def __init__(self, max_allocated_memory_gb: Union[float, int] = None, config: Optional[Config] = None):
    ...
    def other_memory_sequence(self, data: List['MemoryBlock']):
        ...
        for mem_block in data:
            ...
            if _sim_block.free_time is not None and _sim_block.free_time not in _time_base_blocks:
        ...
        _sorted_time_base_blocks = dict(sorted(_time_base_blocks.items(), key=lambda item: item[0]))
        return _sorted_time_base_blocks

    def simulate(self, data_analysis: Union[List['MemoryBlock']], segment_plot: bool = False) -> CachingAllocator:
            ...
            try:
                for _sub_index, _block in enumerate(mlloc_first_blocks):
                    ...
                    if not _block.is_allocated and _block.allocate_block is None:
                        _alloc_block = self._allocator.malloc(_device, _stream, _block.size)
                        _block.allocate_block = _alloc_block
                        _block .is_allocated = True
                        _alloc_size = _alloc_block.size if _alloc_block.requested_size is None else _alloc_block.requested_size
                        if _block.size != _alloc_size:
                            print(f"Block size is not equal to allocated size: {_block.size} != {_alloc_size}")
                    ...
                    if segment_plot:
                        ...
                        _snapshot_img = os.path.join(_snapshot_dir, f'frame-{_index}-{_sub_index}.png')
                ...
                _snapshot_files.append(_snapshot_img)
                self._allocator.plot(_snapshot_img)
                print(f"Memory allocation failed at block {_index}/{_sub_index}\nthe memory capture is saved at {_snapshot_img}")
