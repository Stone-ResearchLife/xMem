...
class Estimator:
    ...
    def data_memory(self) -> List[MemoryBlock]:
        ...
        return [self.create_memory_block(tensor.nbytes) for tensor in data if isinstance(tensor, torch.Tensor)]
    ...
    def get_dataset_memory(self, iteration_index: int, need_released: bool = False) -> List[MemoryBlock]:
    ...
    def get_an_iteration_memory(self, iteration_index: int, zero_grad: bool = False) -> List[MemoryBlock]:
        assert iteration_index > 0
        assert isinstance(iteration_index, int)
        memory_activity: List[MemoryBlock] = []
        iteration = self.profiler.get_iteration(iteration_index)
            ...
            for mem in layers:
                ...
                for backward in mem.get("backward_memory", []):
                    ...
                    if backward.free_time is None:
                        ...
                        end_mem_cpu_instant._value['args']['Addr'] = backward._start.address
    ...
    def estimate(self, target_iteration: int = 2) -> Tuple[CachingAllocator, dict]:
        ...
        if target_iteration > 1:
            ...
            target_iteration_memory = self.profiler.get_iteration(target_iteration).get_memory_activities()
        ...
        for iteration in range(1, target_iteration + 1):
            ...
            memory_activity += self.get_dataset_memory(
                ...
                need_released=dataset_need_released
            )
        ...
        sorted_memory = copy.deepcopy(sorted(memory_activity, key=lambda x: x.alloc_time))
